...
func CreateSimulateData(w http.ResponseWriter, r *http.Request) {
	...
	process, ok := requestData["process"]
	if !ok {
		http.Error(w, "Falta el campo 'process' en la solicitud", http.StatusBadRequest)
		return
	}

	state, ok := requestData["state"]
	if !ok {
		http.Error(w, "Falta el campo 'state' en la solicitud", http.StatusBadRequest)
		return
	}

	// Obtener la fecha y hora actual
	currentTime := time.Now()
	fechaHora := currentTime.Format("2006-01-02 15:04:05")

	// Insertar los datos en la tabla simulate de la base de datos KERNEL
	_, err = db.Exec("INSERT INTO simulate (process, state, fecha_hora) VALUES (?, ?, ?)", process, state, fechaHora)
	if err != nil {
	...
	w.Write([]byte("Datos insertados correctamente en la base de datos"))
}
...
func main() {
	...
	// Endpoint para crear datos en la tabla simulate
	mux.HandleFunc("/simulate/addprocess", func(w http.ResponseWriter, r *http.Request) {
		CreateSimulateData(w, r)
	})
